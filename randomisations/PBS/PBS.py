...
stdout.flush()
gt_outgroup = allel.GenotypeDaskArray(ds_snps_outgroup["call_genotype"].data)
outgroup_segregating_mask = gt_outgroup.count_alleles().is_segregating()
gt_outgroup = gt_outgroup.compress(outgroup_segregating_mask, axis = 0)
...
gt = gt.compress(segregating_mask, axis = 0).compress(outgroup_segregating_mask, axis = 0)
...
true_pbs_df = pd.DataFrame({'startpoint': startpoints, 'endpoints': endpoints, 'midpoint':midpoints, 'pbs':pbs})
...
for i in np.arange(1,num_randomisations+1):
    ...
    if Path(f"randomisations/PBS/PBS_{cohort}_{i}.{contig}.tsv").exists():
    ...
    stdout.flush()
    pbs = allel.pbs(ac1=ac1, ac2=ac2, ac3=ac_outgroup, window_size=window_size, normed=True)
    pbs_df = pd.DataFrame({'midpoints':midpoints, 'pbs':pbs})
    pbs_df.to_csv(f"PBS_{cohort}_{i}.{contig}.tsv", sep="\t")
    randomised_pbs[f"r{i}"] = pbs
