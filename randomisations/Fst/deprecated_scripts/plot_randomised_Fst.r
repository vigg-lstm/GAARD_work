...
plot.randomised.fst <- function(fst.table, pos.table, filename = NULL, plot.title = '', gaps = 5000000){
	...
	layout(matrix(c(rep(1,5), rep(2,1)), nrow = 6, ncol = 1))
	par(mar = c(0,4,1,4), mgp = c(2, 0.7, 0), family = 'Arial', xpd = NA) 
	colours <- c(fst = 'orangered3', randomisations = 'grey50')
	# Create the empty plot
	max.y <- max(c(max(fst.table[, 3:ncol(fst.table)]), 0.05))
	min.y <- min(fst.table[, 3:ncol(fst.table)])
	...
	# Add randomised data
	randomisation.names <- colnames(fst.table)[4:ncol(fst.table)]
	sapply(randomisation.names, function(x) fst.table[, lines(genome.pos, get(x), col = colours['randomisations'], lwd = 0.4), by = window.chrom])
	# Add true data
	...
	par(mar = c(1,4,0,4), mgp = c(2, 0.7, 0)) 
