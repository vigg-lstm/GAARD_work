...
study.pop = 'Obuasi_gambiae_Delta'
...
# Looks like big peaks around Cyp6aa and maybe near Gste
...
clump1 <- list(clump.id = 'clump1',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2R', 283:285, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2R',
               start = 28250000,
               end = 28550000
)
...
clump2 <- list(clump.id = 'clump2',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2R', 289, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2R',
               start = 28850000,
               end = 28950000
)
...
clump3 <- list(clump.id = 'clump3',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 276:277, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 27550000,
               end = 27750000
)
...
clump4 <- list(clump.id = 'clump4',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3R', 150, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3R',
               start = 14950000,
               end = 15050000
)
...
clump5 <- list(clump.id = 'clump5',
			   snp.ids = as.character(top.snps[approx.pos %in% paste('3R', 309, sep = ':'), ][order(Pos), ]$ID),
			   chrom = '3R',
			   start = 30850000,
			   end = 30950000
)
clump5$genes <- unique(str_extract(subset(gff, (seqid == clump5$chrom) & 
											   (start < clump5$end) & 
											   (end > clump5$start) &
											   type == 'gene')$attributes,
								   'AGAP\\d{6}'
...
# Draw the genes in the region of the cluster 
draw.gene.model <- function(region.chrom, region.coords, y = 0, text.cex = 0.8){
	genes.in.region <- subset(gff, (seqid == region.chrom) & (start < region.coords[2]) & (end > region.coords[1]) & type == 'gene')
	genes.in.region$agap <- str_extract(genes.in.region$attributes, 'AGAP\\d{6}')
	gene.names <- str_to_title(
		str_extract(
			genes.in.region$attributes, '(?<=;Name=)[^;]+'
		)
	)
	genes.in.region$name <- ifelse(is.na(gene.names), genes.in.region$agap, gene.names)
	gr <- par('usr')
	lines(c(gr[1], gr[2]), c(y, y), lwd = 2, col = 'grey20')
	v.adj = (gr[4] - gr[3])/20
	h.adj = (gr[2] - gr[1])/50
	# Draw the genes
	rect(as.matrix(genes.in.region[,'start']), y - v.adj, as.matrix(genes.in.region[,'end']), y + v.adj, col = 'white', border = 'black')
	text(apply(genes.in.region[,c('start', 'end')], 1, mean), y - 3*v.adj, genes.in.region$name, adj = 1, cex = 0.3, srt = 45)
	# For the exons, keep only isoform A of each gene
	exons.in.region <- subset(gff, (seqid == region.chrom) & (start < region.coords[2]) & (end > region.coords[1]) & type == 'exon' & grepl('Name=AGAP\\d{6}-RA', attributes))
	rect(as.matrix(exons.in.region[,'start']), y - v.adj, as.matrix(exons.in.region[,'end']), y + v.adj, col = 'black', border = NA)
}

plot.significance.along.region <- function(significance.value, significance.direction, snp.pos, filename, chrom.col = NULL, lwd = 0.3, ...){
	# Sanity checks
	if (length(significance.value) != nrow(snp.pos))
		stop('Number of points in significance.value is different to rows in snp.pos.')
	...
	snp.chrom <- unique(snp.pos$Chrom)
	if (length(snp.chrom) != 1)
		stop('All SNPs should be on the same chromosome')
	...
	coords.1 <- par('usr')	
	xrange.1 <- coords.1[2] - coords.1[1]
	# Plot the chromosomes in the region
	top.margin <- 2
	par(mar = c(2.5, 3, top.margin, 1), xpd = NA, mgp = c(1.3, 0.3, 0), ...)
	snp.region <- c(min(snp.pos$Pos), max(snp.pos$Pos))
	plot(snp.region, c(0, 1.6), type = 'n', bty = 'n', xlab = paste('Position on', snp.chrom), ylab = '', yaxt = 'n', cex.axis = 0.8, tcl = -0.25, cex.lab = 0.8)
	draw.gene.model(snp.chrom, snp.region, 1.5)
	coords.2 <- par('usr')
	xrange.2 <- coords.2[2] - coords.2[1]
	# Write a function to convert old x coords to new x coords
	f <- 1
...
focal.snps <- top.snps[ID %in% clump3$snp.ids]
#plot.significance.along.region(focal.snps$logregP, focal.snps$logreg.direction, focal.snps[, c('Chrom', 'Pos')])
plot.significance.along.region(focal.snps$logregP, focal.snps$logreg.direction, focal.snps[, c('Chrom', 'Pos')], filename = 'sig_along_Cyp6.png', family = 'Arial')

save.image('classical_analysis_nosibs.Rdata')


plot.table <- data.frame(combined.snps = as.numeric(top.snp.clumps[approx.pos == '2R:283', ..sample.names]),
                         phenotype = phenotypes)

bplot <- ggplot(plot.table, aes(x = phenotypes, y = combined.snps, colour = phenotypes)) +
         geom_boxplot(lwd = 1) +
		 geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = 0.4) + 
		 theme_bw() + 
		 theme(text = element_text(size = 20, family = 'Arial'), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
ggsave('2R_cluster_boxplot.png')
