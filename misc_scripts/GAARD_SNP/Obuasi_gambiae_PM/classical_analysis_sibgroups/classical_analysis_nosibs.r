...
# There were no significant SNPs. 
...
# A few notable peaks. The biggest is on 3L. Ace1 is there but not that tall.
...
clump1 <- list(clump.id = 'clump1',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 372, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 37150000,
               end = 37250000
)
...
clump2 <- list(clump.id = 'clump2',
               snp.ids = as.character(top.snps[approx.pos == '2R:35', ][order(Pos), ]$ID),
               chrom = '2R',
               start = 3450000,
               end = 3550000
)
...
clump3 <- list(clump.id = 'clump3',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2R', 276:277, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2R',
               start = 27550000,
               end = 27750000
)
...
clump4 <- list(clump.id = 'clump4',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2R', 338:339, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2R',
               start = 33750000,
               end = 33950000
)
...
clump5 <- list(clump.id = 'clump5',
               snp.ids = as.character(top.snps[approx.pos == '2R:384', ][order(Pos), ]$ID),
               chrom = '2R',
               start = 38350000,
               end = 38450000
)
...
clump6 <- list(clump.id = 'clump6',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 112, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 11150000,
               end = 11250000
)
clump6$genes <- unique(str_extract(subset(gff, (seqid == clump6$chrom) & 
                                               (start < clump6$end) & 
                                               (end > clump6$start) &
                                               type == 'gene')$attributes,
                                   'AGAP\\d{6}'
...
clump7 <- list(clump.id = 'clump7',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 114, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 11350000,
               end = 11450000
)
clump7$genes <- unique(str_extract(subset(gff, (seqid == clump7$chrom) & 
                                               (start < clump7$end) & 
                                               (end > clump7$start) &
                                               type == 'gene')$attributes,
                                   'AGAP\\d{6}'
...
clump8 <- list(clump.id = 'clump8',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 248:249, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 24750000,
               end = 24950000
)
clump8$genes <- unique(str_extract(subset(gff, (seqid == clump8$chrom) & 
                                               (start < clump8$end) & 
                                               (end > clump8$start) &
                                               type == 'gene')$attributes,
                                   'AGAP\\d{6}'
...
clump9 <- list(clump.id = 'clump9',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 305:307, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 30450000,
               end = 30750000
)
clump9$genes <- unique(str_extract(subset(gff, (seqid == clump9$chrom) & 
                                               (start < clump9$end) & 
                                               (end > clump9$start) &
                                               type == 'gene')$attributes,
                                   'AGAP\\d{6}'
...
clump10 <- list(clump.id = 'clump10',
               snp.ids = as.character(top.snps[approx.pos == 'X:82', ][order(Pos), ]$ID),
               chrom = 'X',
               start = 8150000,
               end = 8250000
)
clump10$genes <- unique(str_extract(subset(gff, (seqid == clump10$chrom) & 
                                               (start < clump10$end) & 
                                               (end > clump10$start) &
                                               type == 'gene')$attributes,
                                   'AGAP\\d{6}'
...
top.snp.clumps.ac <- allele.counts.table[unlist(lapply(clumps, '[[', 'snp.ids'))]
...
# We just get a big 2La signal and everything else is mostly scattered. 
...
# Let's load up the Ace1 Dup calls. For the kmers, we had used modal coverage because the kmers
# are quantitative. Here we use presence / absence because the SNPs are qualitative. 
...
# There is no point taking the correlation with Dup1, because every sample in this dataset has Dup1
...
png('Del1_correlation_vs_significance.png', width = 8, height = 5, units = 'in', res = 300)
par(mfrow = c(1,2), family = 'Arial')
plot(abs(top.snps$Del1.cor), -log(top.snps$logregP), pch = 21, col = 'grey50', bg = chrom.colours[top.snps$Chrom])
plot(abs(non.ace1.snps$Del1.cor), -log(non.ace1.snps$logregP), pch = 21, col = 'grey50', bg = chrom.colours[non.ace1.snps$Chrom])
dev.off()
