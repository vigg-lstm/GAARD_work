...
study.pop = 'Madina_gambiae_Delta'
...
clump1 <- list(clump.id = 'clump1',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 236, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 23550000,
               end = 23650000
)
...
clump2 <- list(clump.id = 'clump2',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 266, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 26550000,
               end = 26650000
)
...
clump3 <- list(clump.id = 'clump3',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 397:399, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 39650000,
               end = 39950000
)
...
clump4 <- list(clump.id = 'clump4',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 308, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 30750000,
               end = 30850000
)
