...
study.pop = 'Avrankou_coluzzii_Delta'
...
# There is a clear 2La signal, and peaks on 3R and 3L.
...
# There are two clusters outside of 2La:
clump1 <- list(clump.id = 'clump1',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 160, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 15950000,
               end = 16050000
)
...
clump2 <- list(clump.id = 'clump2',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3R', 421, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3R',
               start = 42050000,
               end = 42150000
)
