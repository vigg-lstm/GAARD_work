...
# There are some clear peaks here
...
# There is a large group on 2R
# That's a large region with a lot of genes. Let's pull them all out
clump1 <- list(clump.id = 'clump1',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 38:39, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 3750000,
               end = 3950000
)
...
clump2 <- list(clump.id = 'clump2',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 45, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 4450000,
               end = 4550000
)
...
clump3 <- list(clump.id = 'clump3',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 241:242, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 24050000,
               end = 24250000
)
...
clump4 <- list(clump.id = 'clump4',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 392, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 39150000,
               end = 39250000
)
...
clump5 <- list(clump.id = 'clump5',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2L', 488:489, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2L',
               start = 48750000,
               end = 48950000
)
...
clump6 <- list(clump.id = 'clump6',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2R', 120:121, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2R',
               start = 11950000,
               end = 12150000
)
...
clump7 <- list(clump.id = 'clump7',
               snp.ids = as.character(top.snps[approx.pos %in% paste('2R', 210, sep = ':'), ][order(Pos), ]$ID),
               chrom = '2R',
               start = 20950000,
               end = 21050000
)
...
clump8 <- list(clump.ids = 'clump8',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 257, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 25650000,
               end = 25750000
)
...
clump9 <- list(clump.id = 'clump9',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3L', 317, sep = ':'), ][order(Pos), ]$ID),
               chrom = '3L',
               start = 31650000,
               end = 31750000
)
...
clump10 <- list(clump.id = 'clump10',
               snp.ids = as.character(top.snps[approx.pos %in% paste('3R', 43, sep = ':'), ][order(Pos), ]$ID),
                chrom = '3R',
                start = 4250000,
                end = 4350000
)
...
clump11 <- list(clump.id = 'clump11',
               snp.ids = as.character(top.snps[approx.pos %in% paste('X', 6, sep = ':'), ][order(Pos), ]$ID),
                chrom = 'X',
                start = 550000,
                end = 650000
)
clump11$genes <- unique(str_extract(subset(gff, (seqid == clump11$chrom) & 
                                                (start < clump11$end) & 
                                                (end > clump11$start) &
                                                type == 'gene')$attributes,
                                    'AGAP\\d{6}'
...
clump12 <- list(clump.id = 'clump12',
               snp.ids = as.character(top.snps[approx.pos %in% paste('X', 14, sep = ':'), ][order(Pos), ]$ID),
                chrom = 'X',
                start = 1350000,
                end = 1450000
)
clump12$genes <- unique(str_extract(subset(gff, (seqid == clump12$chrom) & 
                                                (start < clump12$end) & 
                                                (end > clump12$start) &
                                                type == 'gene')$attributes,
                                    'AGAP\\d{6}'
