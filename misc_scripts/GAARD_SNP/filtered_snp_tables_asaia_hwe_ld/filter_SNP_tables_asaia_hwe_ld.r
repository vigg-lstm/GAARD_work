...
test.train.split <- fread('../data/test_train_split.csv', key = 'sample.name')
...
# One-tailed Hardy Weinberg test (checking for heterozygote excess)
...
load.SNPs <- function(pop){
	# Load SNP data after accessisibility filtering and LD pruning. 
	SNP.filename <- paste('..', pop, 'csvs/filtered_SNPs_acc_pruned.csv', sep = '/')
	...
	training.set <- test.train.split[sample.names][is.train == T]
	names.train <- training.set$sample.name
	phen.train <- training.set$phenotype
	...
	# Filter out reads with heterozygote excess
	snp.table[, hwe := apply(.SD, 1, HW.test), .SDcols = sample.names]
	snp.table <- snp.table[hwe > 0.05, ]
	...
	cat('Calculating logistic regression P-value using only training set samples.\n')
	snp.table[, logregP.train := future_apply(.SD, 1, logreg.test, phenotype = factor(phen.train, levels = c('alive', 'dead'))), .SDcols = names.train]
	snp.table$logregP.train[is.na(snp.table$logregP.train)] <- 1
...
for (pop in pops)
	saveRDS(snp.tables[[pop]], paste('filtered_snp_tables_asaia_hwe_ld_', pop, '.rds', sep = ''))
